...
\label{lhc_intro}
%LHC intro 
The LHC is a 27 km circular circumference storage ring, accelerator and collider for 
...
luminosity of up to $10^{34}\,\cm^{-2}\rm{s}^{-1}$. This makes the LHC the only collider
...
a 400 MHz superconducting cavity system. The beams of protons or lead ions 
...
provide around 275~kW of RF power independently to each beam to accelerate the protons
...
discovery of the Higgs boson~\cite{cmsHiggs,atlasHiggs} as well as large regions of 
the parameter space of many BSM physics models to be probed. From 2013 to mid-2015 
(Long Shutdown 1) the LHC was shut down for upgrade to allow design
...
As shown in Figure~\ref{fig:LHC-integrated-lumi}, by July 2016, $15.0\,\ifb$ of integrated luminosity was
...
The (right-handed) coordinate system used by CMS takes the origin at 
the collision point. The z-axis points along the anticlockwise beam direction while the x-axis points radially inward
...
particles is approximately invariant under boosts along the z-axis~\cite{cms_iop}. 
...
\centering
  ...
  through different segments of the detector~\cite{CMS}}.
  \label{CMS_SLICE}
\end{figure}
...
which measures energy deposited in electromagnetic showers from particles such as electrons and photons and is separated into barrel and endcap 
components for the central and forward regions respectively. The hadronic calorimeter (HCAL) lies outside the 
ECAL and measures the energy of hadronic particles. It is a sampling calorimeter
...
Muons deposit little energy within the detector, and typically reach the cavern containing the detector. 
...
\section{Tracker}
The CMS tracker is an all-silicon tracker with an area sensitive to charged particles of around 200 $\rm{m}^2$. It is designed to measure hits along the
curved trajectories of charged particles that result from the high energy p-p collisions. A cross section of the tracker in the $r$-$z$ plane is shown in Figure~\ref{TRACKER_SLICE}.
...
Close to the interaction vertex, $r < 20~\cm$, where the particle flux is maximal ($~10^7/\rm{s}$), is the pixel detector. This system consists of an inner region of 66 million silicon 
...
for $r > 20\cm$ is covered by endcap discs divided into the 9 discs of the tracker end cap (TEC) for the $120 \cm < |z| < 280 \cm$ region and 
3 tracker inner discs (TID) lying between the TIB and TEC. The TID and first 3 discs of the TEC have sensors of thickness $320\mum$ while the remainder of the TEC discs
...
and two ECAL endcaps (EE) outside the tracker~\cite{ecal_tdr}. The high density (8.28 $\rm{g}/\cm^3$), short radiation length (0.89$\cm$) 
...
and photodiodes requires a stable temperature of $\sim291\,\rm{K}$ with a target stability of $0.1\,\rm{K}$. 

The final component of the ECAL is the preshower (ES) which covers much of the endcaps ($1.653 < |\eta| < 2.6$). In addition
to its principle aim of identifying neutral pions, it assists in identifying electrons and in
...
\begin{equation}
\frac{\sigma_E}{E} = \frac{a}{\sqrt{E(\GeV)}} \oplus \frac{b}{E(\GeV)} \oplus c,
\label{equ:energy-resolution}
\end{equation}

\noindent where the parameters $a$, $b$, $c$ are the stochastic, noise and constant contributions respectively. These have been
...
the ECAL and magnet. The HB is composed of 17 layers of brass interleaved with plastic scintillar tiles. 
Brass is used as the absorbing material due to its relatively short interaction length (16.42$\cm$), ease of machining 
and because it is non-magnetic. The space taken by the active material is minimised through plastic scintillator tiles
read out with embedded wavelength-shifting (WLS) fibres~\cite{CMS}. The detector is made up of 36 identical azimuthal wedges split into two barrels.
The active scintillator tiles are divided into 2304 segments (towers), each covering $\Delta\phi \times \Delta\eta = 0.087 \times 0.087$, corresponding to the 
...
12 sections of $30^\circ$~\cite{hcal_tdr}. 

Finally, coverage for $3.0 < |\eta| < 5.0$ is provided by the HF installed $11.2\,\rm{m}$ from the interaction point. This provides
excellent measurement of forward hadronic activity as well as hermeticity for \met measurements~\cite{hcal_tdr}. The HF is composed of a 1.65~m thick steel absorber 
...
\begin{equation}
\frac{\sigma_H}{E} = \frac{94.3\%}{\sqrt{E(\GeV)}} \oplus 8.4\%.
...
read to tape to be fully reconstructed. A schematic diagram of the DAQ process is show in Figure~\ref{fig:DAQ_SLICE}. 
...
require the use of simplified reconstruction algorithms. The reconstruction is performed using trigger paths
...
The fragmentation and hadronisation of the coloured particles is simulated using dedicated showering and hadronisation algorithms with
