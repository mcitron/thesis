...
The \alphat~analysis is an hadronic search for new physics which 
targets models which are produced and decay via the strong force.
Such modes may be expected to dominate in the proton-proton collisions 
at the LHC. Exploiting an hadronic final state requires effecitive 
...
The \alphat~analysis~\emph{signal region} selects a final state 
including jets and significant \met with no reconstructed leptons or photons
to search for new physics. The QCD multijet process may pass such 
a selection through the mechanisms described in Section~\ref{sec:qcd-background-intro}. 
To mitigate this otherwise 
dominant background, dedicated dimensionless variables, described in Section~\ref{sec:important-variables}, 
are used to suppress the QCD multijet process in the signal region to the percentage level. 
In defining the signal region, additional hadronic, kinematic and cleaning selections 
are used for further QCD and electroweak background suppression and are described in section~\ref{sec:had_sr}.

The determination of residual QCD multijet backgrounds as well as backgrounds with 
genuine \met~(described in Section~\ref{sec:ewk-background-intro}) relies on data driven techniques described in Sections ?? and ?? respectively.
...
There are several SM processes which form backgrounds for hadronic searches. These include 
true~\met from neutrinos and are termed the \emph{electroweak} backgrounds. The dominant 
electroweak background processes are \wj,\ttj and \zj. An overview of these processes 
and the mechanisms by which they may enter into the hadronic signal region 
(defined in Section~\ref{sec:had_sr}) are described below. 

\subsubsection{\zj background}

The largest background process in the $\alphat$ signal region comes from Z production
in association with jets (\zj) followed by decay to neutrinos, \znunu. This process is an \emph{irreducible} background
as the neutrinos can cause significant \met in the final state. Across the signal region
\znunu contributes $\sim 54\%$ of the overall background. The decay of the Z to leptons, \zll may also
introduce $\met$ if one or both of the leptons is not reconstructed (\emph{lost}). However, as
an hadronic final state is required, this background is sub-dominant, comprising $\sim 0.5\%$
of the signal region, due to the low probability of failing to reconstruct both leptons.

\subsubsection{\wj background}

The \wj background comprises $\sim 38\%$ the overall background in the signal region.
Purely leptonic decays of the W may pass hadronic selection if the lepton is not reconstructed. 
...
The final state in an event with \ttj production may include multiple jets (up to six
if both tops decay hadronically) and/or significant \met depending on the decay of 
the tops as well as two bottom quarks. If both W bosons decay hadronically ($\sim45.7\%$ of \ttbar~decays)
...
W bosons decays leptonically ($\sim45.8\%$ of \ttbar~decays) up to four jets may be expected to be produced in the decay
...
and lack of jets in the final state. In all cases the number of jets may be increased by processes such 
as ISR and FSR.
...
In addition to the backgrounds discussed above, there are several additional processes which 
make minor contributions to the overall background. These contribution can be enhanced, however, 
for particular categorisations. Such residual backgrounds include \ttbar production in association with a vector
boson, comprising $\sim0.1\%$ of the total background, which rises to $\sim4\%$ for an $\nb \ge 2$ selection;
pair production of vector bosons (ZZ, WZ and WW), comprising $\sim1.4\%$ of the total background; production
of a photon in association with jets where the photon is lost, comprising $\sim0.7\%$ of the overall background and
leptonic decays of the Z with two lost leptons (0.5\% of the overall background).
...
Isolated tracks are used to identify W boson decays and single prong decays of the $\tau$ (see Section ??). 
