...
containing additional jets with $\pt > 25 \GeV$ the \bdphi variable (and no other variable) 
is defined including these jets. For events with no addition jets with $\pt > 25 \GeV$
...
the $\mht > 130 \GeV$ threshold in both signal and control helps ensure the Z is sufficiently
...
provided to the visible decay products. The finest control and signal region 
binning in \scalht~is shown in~\ref{tab:ht-binning}. These are merged from 
above depending on the requirement of at least one control region
...
In the categorisation, a maximum statistical uncertainty from the simulated events of 50\% (corresponding to four unweighted events) 
is required in each bin of the template in order to ensure a statistically meaningful prediction. 
In addition, a minimum bin width constraint of 50 GeV is applied in order to reduce the bin-by-bin migration 
...
data selected by indepedent muon and electron \emph{reference} triggers to pass signal 
region selection. For the case of the muon reference trigger the muon is cross-cleaned 
such that jet based energy sums do not include the muon. For the electrons this 
cross-cleaning is not applied. The central value of the trigger efficiency
is taken from the measurement using the electron reference trigger. The efficiency in \mht~
bins for different \scalht~ranges are shown in Figure ??. Given the selection of $\mht > 200\GeV$,
an efficiency of $>90\%$ is acheived across the signal region. 

Biases may be introduced in the trigger efficiency measurement due to contamination in the computation
of event variables and different treatments between trigger and offline reconstructions.
...
The \gj control region is selected using the HLT seeds ?? and L1 seed ??. The efficiency is 
measured in data using events in the JetHt primary dataset which satisfy the \gj control 
region requirements. These efficiencies are showin in Figure ?? and applied per photon \pt.
