...
$b^{j,i}_{\text{QCD},\mathrm{had}}$ is the predicted 
...
to the control regions (see below); $\phi^{j,i}$ contains the systematic uncertainties on the 
transfer factors from the data-driven tests; $\rho^{j}$ contains the systematics from 
...
$r$ is the unconstrained `signal strength' parameter and $\omega_{\text{QCD},\mathrm{had}}$ 
...
similarly to Equation~\ref{eq:hadronicLikelihood}, $\rho^{j}_{\mu}$ contains the uncertainty in the \htcat from variations in simulation. The $s^{j}_{\mathrm{\mu}}$
encodes the signal contamination in the control region, which is small by design. The connection between the control and signal region
is encoded by the unconstrained $a^{j}$ parameter and the subdominant QCD component, $b^{j}_{\text{QCD},\,\mu}$, is taken from simulation. 
...
In these equations the $\phi^{j}$ appear on the denominator because the connection between the control and signal region is 
inverted.

The modifier and constraint terms of the parameters representing the systematic uncertainties and 
connections between control and signal regions, `nuisance parameters' , can be summarised as:
\begin{itemize}
...
two parameters per bin that multiply the total background and signal contributions and that
...
on each of these corrections, analogously to the systematic uncertanties on the background processes dervied from
...
Also shown are the expected and observed contours of $r_{95} = 1$ (models `excluded'
