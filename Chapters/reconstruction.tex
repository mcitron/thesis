...
measured in early LHC data and compared with simulation as shown in Figure~\ref{fig:pvEffRes}.
...
\item the collision to occur within a radial distance of $|d_{xy}| < 2\,\cm$ from the beamline
\end{itemize}
...
for the HCAL components are derived during LHC running~\cite{hcal_calib}.
...
\begin{figure}
\centering
    \includegraphics[width=0.6\textwidth]{./Figures/reconstruction/electron_ecal.png}
...
In order to ensure the muons are prompt (produced by a hard process such as vector boson decay) rather
...
overall uncertainty is shown in Figure~\ref{fig:jec_unc}.
...
\centering
  ...
  The peak in the pileup component of the uncertainty between 100 and 200 \GeV is caused by taking the absolute value of 
  the log-linear parameterisation of this component.}
\end{figure}
