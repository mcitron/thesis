...
\begin{figure}[hbt]
  \begin{center} 
   \subfigure[\label{fig:pvEff}]{\includegraphics[width=0.45\textwidth]{Figures/detector/pvEff}}~
   \subfigure[\label{fig:pvRes}]{\includegraphics[width=0.45\textwidth]{Figures/detector/pvRes}}
...
the inputs used to reconstruct particles and jets. These \emph{physics objects}
...
\begin{equation}
\metvec^{\,\rm{corr}} = \metvec - \sum_{j}(\vec{p}^{\,\text{corr},j}_\text{T} -
\vec{p}_\text{T}^{\,j}).
\end{equation}

This \emph{type-1} correction~\cite{met_fig}~uses all corrected jets with $\pt > 15\GeV$ with less than 0.9 of their energy deposited in the ECAL.
...
with a cone size of $\Delta R = 0.4$ for muons in the control regions using a PF mini-isolation requirement of
